...
import Notensor (ProdVector (identityBuilder), BasicVector(..), fstF1, sndF1, intoFst, intoSnd, BackFunc (BackFunc), FullVector, Transpose)
...
type BVar b a v = AffineFunc b (AnyExpr BackFunc a v)
...
backprop' :: forall da dv. (BasicVector da, FullVector dv) => AnyExpr BackFunc da dv -> dv -> da
backprop' dy dv = unsafePerformIO $ do
    g <- runRecoverSharing7 dy -- :: IO (NodeMap.SomeSharedExprWithMap BackFunc da dv)
...
fstF1' :: forall u v. ProdVector u => BackFunc (u, v) (SparseVector u)
fstF1' = BackFunc back
...
fst (AffineFunc (y0, _y1) (AnyExpr dy0)) = AffineFunc y0 (castNode node)
    where node :: Expr BackFunc a (SparseVector u)
...
liftFunc1 
  :: forall u v da dv du. 
     BasicVector dv
  => (u -> (v, BackFunc du dv))
  -> AffineFunc u (AnyExpr BackFunc da du)
  -> AffineFunc v (AnyExpr BackFunc da dv)
