...
data ForwardNode s da dz dx where
    ForwardNode :: BasicVector dx => [ForwardEdge s da dz dx] -> ForwardNode s da dz dx
...
data BackwardNode s da dz dx where
    BackwardNode :: BasicVector dx => [BackwardEdge s da dz dx] -> BackwardNode s da dz dx
...
convertGraph env (ExprSum zs) = ForwardGraph (NodeMap.mapmap convertInnerNode env) (ForwardNode (convertFinalEdge <$> zs))
    where convertInnerNode :: forall dx. SharedExprS s da dx -> ForwardNode s da dz dx
          convertInnerNode (ExprSum xs) = ForwardNode (convertInnerEdge <$> xs)
...
evalBackMap dxs dz = ys
    where ys = NodeMap.mapmap go dxs
          ...
          go (BackwardNode xs') = BackValue (sumBuilder (goEdge <$> xs'))
...
instance BasicVector da => TensorProduct dz (BackwardGraph s da dz) da where
    dx ⊗ BackwardGraph env (BackwardNode edges) = sumBuilder (go <$> edges)
...
forwardNodeEdges name (ForwardNode xs) = go <$> xs
...
allFwdEdges (ForwardGraph env (ForwardNode es)) = finalEdges ++ innerEdges
...
backFromEdges dictmap edges = BackwardGraph edgeMap (BackwardNode initial)
    where (initial, inner) = partitionEithers (classifyBackEdge <$> edges)
          ...
          withDict NodeDict (List2 xs) = BackwardNode xs
