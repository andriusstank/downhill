...
newtype BackGrad da v = BackGrad (forall x. (x -> DualBuilder v) -> [Term BackFun da x])

-- | Creates a @BackGrad@ that is backed by a real node. Gradient of type '@DualOf@ v' will be computed for this node.
realNode :: Expr BackFun da (DualOf v) -> BackGrad da v
realNode x = BackGrad (\f -> [Term (BackFun f) x])

-- | Type of a node can be changed freely, as long as its @VecBuilder@ stays the same.
castNode :: forall dr dv z. (BasicVector dv, DualBuilder z ~ VecBuilder dv) => Expr BackFun dr dv -> BackGrad dr z
castNode node = BackGrad go
  where
    go :: forall x. (x -> DualBuilder z) -> [Term BackFun dr x]
...
inlineNode :: forall da u v. (DualBuilder v -> DualBuilder u) -> BackGrad da u -> BackGrad da v
inlineNode f (BackGrad g) = BackGrad go
  where
    go :: forall x. (x -> DualBuilder v) -> [Term BackFun da x]
    go h = g (f . h)

instance (HasDual v) => AdditiveGroup (BackGrad da v) where
...
instance HasDual v => VectorSpace (BackGrad da v) where
  type Scalar (BackGrad da v) = Scalar (DualOf v)
