...
instance FullVector dv => AdditiveGroup (Expr5 AFunction1 da dv) where
    zeroV = zeroE
...
instance FullVector dv => VectorSpace (Expr5 AFunction1 da dv) where
    type Scalar (Expr5 AFunction1 da dv) = Scalar dv
    a *^ v = Expr5 (Node [Edge (scaleFunc a) (InnerNode v)])

instance FullVector dv => AdditiveGroup (Endpoint (Expr5 AFunction1 da) da dv) where
...
instance FullVector dv => VectorSpace (Endpoint (Expr5 AFunction1 da) da dv) where
    type Scalar (Endpoint (Expr5 AFunction1 da) da dv) = Scalar (Expr5 AFunction1 da dv)
    a *^ x = InnerNode (Expr5 (Node [Edge (scaleFunc a) x]))

sumExpr2 :: FullVector dv => [Expr5 AFunction1 da dv] -> Expr5 AFunction1 da dv
